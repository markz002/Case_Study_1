...
My approach is that for each secondary outcome domain, participants were grouped by whether they ordered an HIV self-test kit (ora_within60_yesno). Summary statistics were calculated separately for those who ordered a kit and those who did not. Depending on the nature of the variable. For categorical or binary likert variables were summarized using counts and percentages, and compared using Fisher's exact test. For multi-level likert variables, they were also summarized using counts and percentages, but compared using Wilcoxon rank sum test. While continuous variables were summarized using means and standard deviations, and compared using the Student's t-test. My use of test follows the original study's appendix 3.
